...
analytics foundation`, and `Phase 3B.17 internal tournament runtime` are now
...
0. APOLLO launch expansion fork, if chosen next
   - the canonical operating doc is
     `../apollo/docs/launch-expansion-audit.md`
   - 3B.17 has closed internal tournament runtime only
   - do social safety/reliability, public readiness gates, match tiers, and
     consensus workflow before public competition stakes
...
1. next Phase 3B planning fork after closed `Phase 3B.17`
   ...
   - APOLLO now also owns competition command/outcome/readiness contracts,
     ...
     advancement facts, explicit advance reasons, and tournament event facts
   ...
   - choose 3B.18 social safety/reliability next if continuing launch
     expansion; keep dashboard-first analytics, public profiles/stats/scouting,
     carry coefficient, OpenSkill read-path switch, public competition surfaces,
     public tournaments, CP/badges/rivalry/squads, proposal workflow,
     booking/commercial work, and Hestia public/member competition expansion out
     unless a later packet earns those boundaries
...
The active next ladder is now a post-`Phase 3B.17` planning fork:

| Order | Line | Repo focus | Release line | Purpose | Hard stop |
| --- | --- | --- | --- | --- | --- |
| 1 | `Phase 3B.18` social safety + reliability | likely `apollo` first, Themis/Hestia only if gates require | next if launch expansion continues | prove moderation, reliability, privacy, and operational safety before public competition exposure | no public competition surface, public tournament page, or social stakes by implication |
| 2 | `Phase 3B.19` public competition readiness | likely `apollo`, `themis`, then `hestia` only after gates | later | evaluate public competition and public tournament exposure against real internal facts | no public competition surface before privacy, result trust, rating, telemetry, scale, and compatibility gates |
| 3 | `Phase 3B.20` CP / badges / rivalry / squads | later | later game layer | add game identity only after safety and public readiness gates | no CP, badges, rivalry, squads, bounty, or public records from untrusted results |

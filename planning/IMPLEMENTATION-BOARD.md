...
competition readiness`, and `Phase 3B.20 game identity layer` are now also closed in
...
`/competition` and the member competition surface, while keeping `/app/**` as
the authenticated member app.
...
1. next Phase 3B planning fork after closed `Phase 3B.20`
   ...
   - choose 3B.20.1 cohesion hardening next if continuing launch expansion;
     keep dashboard-first analytics, public profiles/stats/scouting, carry
...
The active next ladder is now a post-`Phase 3B.20` planning fork:

| Order | Line | Repo focus | Release line | Purpose | Hard stop |
| --- | --- | --- | --- | --- | --- |
| 1 | `Phase 3B.20.1` cohesion hardening | later | later hardening line | harden game identity policies, projection determinism, Hestia proxy boundaries, privacy tests, and docs truth if needed | no messaging/chat, broad public social graph, public safety details, public tournaments, or OpenSkill read-path switch by implication |

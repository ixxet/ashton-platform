...
analytics foundation`, `Phase 3B.17 internal tournament runtime`,
	...
	cohesion hardening`, and Rating Policy Wrapper are now also closed in
...
0. APOLLO launch expansion fork, if chosen next
   ...
   - Rating Policy Wrapper is closed locally/runtime in APOLLO: the active
     ...
     unchanged
   - the next rating packet is Rating Policy Simulation / Golden Expansion
     before any OpenSkill read-path switch
...
1. next Phase 3B planning fork after closed `Phase 3B.20`
   ...
   - Themis now owns the internal `/ops/bookings` staff workspace and
	...
	- 3B-LC smoke/stress closeout, Scale Gate Numeric Ceilings, CLI Demo
	  Spine, and Rating Policy Wrapper are closed locally/runtime; the next
	  launch-expansion line should be Rating Policy Simulation / Golden
	  Expansion unless a hardening packet is needed, and should keep
...
remains comparison-only, public tournaments remain blocked, full historical
simulation/backtesting remains deferred, and deployed truth is unchanged.

The active next ladder is now a post-Rating Policy Wrapper planning fork:

| Order | Line | Repo focus | Release line | Purpose | Hard stop |
| --- | --- | --- | --- | --- | --- |
| 1 | Rating Policy Simulation / Golden Expansion | APOLLO | later proof packet | stress rating policy wrapper behavior against broader fixtures and historical/synthetic scenarios before any OpenSkill cutover discussion | no OpenSkill active read path, public OpenSkill values, public tournaments, frontend formulas, deployed truth, or fake validation claims |

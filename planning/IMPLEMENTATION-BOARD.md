...
`Phase 3B.13 rating foundation`, `Phase 3B.14 OpenSkill dual-run`, and
`Phase 3B.15 ARES v2 proposal foundation` are now
...
0. APOLLO launch expansion fork, if chosen next
   - the canonical operating doc is
     `../apollo/docs/launch-expansion-audit.md`
   - 3B.15 has closed ARES v2 proposal/match-preview foundation only
   - do analytics, match tiers, consensus workflow before public competition
     stakes
...
1. next Phase 3B planning fork after closed `Phase 3B.15`
   ...
   - APOLLO now also owns competition command/outcome/readiness contracts,
     ...
     delta budgets and flags, and ARES v2 queue intent/match-preview proposal
   ...
   - choose 3B.16 competition analytics next if continuing launch expansion;
     keep OpenSkill read-path switch, tournament runtime, public competition surfaces,
...
The active next ladder is now a post-`Phase 3B.15` planning fork:

| Order | Line | Repo focus | Release line | Purpose | Hard stop |
| --- | --- | --- | --- | --- | --- |
| 1 | `Phase 3B.16` competition analytics | `apollo` | next if launch expansion continues | inspect trusted result/rating/ARES preview facts without creating public dashboards | no analytics-as-truth before rating trust; no public stakes |
| 2 | tournament/public/game identity ladder | likely `apollo`, `themis`, then `hestia` only after gates | later | add tournament runtime, public competition readiness, and game identity in order | no public competition surface before privacy, result trust, rating, telemetry, scale, and compatibility gates |

...
`Phase 3B.15 ARES v2 proposal foundation`, and `Phase 3B.16 competition
analytics foundation` are now
...
0. APOLLO launch expansion fork, if chosen next
   - the canonical operating doc is
     `../apollo/docs/launch-expansion-audit.md`
   - 3B.16 has closed internal competition analytics foundation only
   - do internal tournament runtime, match tiers, and consensus workflow before
...
1. next Phase 3B planning fork after closed `Phase 3B.16`
   ...
   - APOLLO now also owns competition command/outcome/readiness contracts,
     ...
     codes, and internal derived analytics stat events/projections with
     explicit projection version, confidence, sample size, computed time,
     source match/result, and deterministic watermarks
   ...
   - choose 3B.17 internal tournament runtime next if continuing launch
...
The active next ladder is now a post-`Phase 3B.16` planning fork:

| Order | Line | Repo focus | Release line | Purpose | Hard stop |
| --- | --- | --- | --- | --- | --- |
| 1 | `Phase 3B.17` internal tournament runtime | `apollo`; Themis only if internal read-only visibility is explicitly needed | next if launch expansion continues | add internal tournament containers/runtime over trusted result/rating/analytics facts | no public tournament pages, fake brackets, or public stakes |
| 2 | public/game identity ladder | likely `apollo`, `themis`, then `hestia` only after gates | later | add public competition readiness and game identity in order | no public competition surface before privacy, result trust, rating, telemetry, scale, and compatibility gates |

...
0. APOLLO launch expansion fork, if chosen next
   ...
   - Rating Policy Simulation / Golden Expansion is closed locally/runtime in
     ...
     unchanged
   - the next launch-expansion packet is Frontend Route/API Contract Matrix
...
1. next Phase 3B planning fork after closed `Phase 3B.20`
   ...
   - Themis now owns the internal `/ops/bookings` staff workspace and
	...
	- 3B-LC smoke/stress closeout, Scale Gate Numeric Ceilings, CLI Demo
	  ...
	  Expansion are closed locally/runtime; the next launch-expansion line
	  should be Frontend Route/API Contract Matrix unless a hardening packet is
	  needed, and should keep dashboard-first analytics, public
...
The active next ladder is now a post-rating-proof planning fork:

| Order | Line | Repo focus | Release line | Purpose | Hard stop |
| --- | --- | --- | --- | --- | --- |
| 1 | Frontend Route/API Contract Matrix | Hestia/Themis/platform/APOLLO docs | later matrix packet | freeze route, API, auth, state, and stub status before broader surface work | no frontend-owned formulas, no public tournaments, no OpenSkill active read path, no deployed truth claims |

...
`Phase 3B.13 rating foundation`, and `Phase 3B.14 OpenSkill dual-run` are now
...
0. APOLLO launch expansion fork, if chosen next
   - the canonical operating doc is
     `../apollo/docs/launch-expansion-audit.md`
   - 3B.14 has closed OpenSkill dual-run comparison only
   - do ARES v2, match tiers, consensus workflow before public competition
     stakes
...
1. next Phase 3B planning fork after closed `Phase 3B.14`
   ...
   - APOLLO now also owns competition command/outcome/readiness contracts,
     ...
     projection watermarks, and internal OpenSkill comparison rows/events with
     delta budgets and flags
   ...
   - choose 3B.15 ARES v2 next if continuing launch expansion;
     keep OpenSkill read-path switch, analytics, tournament runtime, public competition surfaces,
...
delta flags. The active rating read path remains the legacy projection. Themis
owns the internal `/ops/competition` shell over APOLLO contracts and does not
own competition result or rating state. Hestia remains unchanged. Deployed
truth is unchanged.

The active next ladder is now a post-`Phase 3B.14` planning fork:

| Order | Line | Repo focus | Release line | Purpose | Hard stop |
| --- | --- | --- | --- | --- | --- |
| 1 | `Phase 3B.15` ARES v2 | `apollo` | next if launch expansion continues | use trusted legacy rating truth plus internal OpenSkill comparison evidence for explainable matchmaking inputs | no OpenSkill read-path switch, public rating claims, analytics dashboard, or public stakes |
| 2 | Analytics ladder | `apollo` | later | use trusted/versioned rating outputs and ARES prerequisites after comparison | no analytics-as-truth before rating trust; no public stakes from unproven ratings |

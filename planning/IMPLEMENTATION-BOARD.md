...
0. APOLLO launch expansion fork, if chosen next
   ...
   - the next launch-expansion packet is ATHENA real ingress bridge or live
     destructive-probe/SIGTERM proof planning, depending on planning ruling
...
1. next Phase 3B planning fork after closed `Phase 3B.20`
   ...
   - Themis now owns the internal `/ops/bookings` staff workspace and
	...
	- 3B-LC smoke/stress closeout, Scale Gate Numeric Ceilings, CLI Demo
	  ...
	  Expansion, and Game Identity Policy Tuning Loop are closed
	  locally/runtime; Frontend Route/API Contract Matrix is closed as docs
	  truth only. The next launch-expansion line should be ATHENA real ingress
	  bridge or live destructive-probe/SIGTERM proof planning unless a hardening
	  packet is needed, and
...
| 3 | ATHENA Real Ingress Bridge | APOLLO/ATHENA/platform planning | later cross-repo packet | strengthen physical truth for sediment teams, daily check-in XP, and reliability verification | no fake ATHENA truth claims, broad social graph, public tournaments, or deploy overclaim |
| 4 | Live destructive-probe and SIGTERM proof plan | APOLLO/platform ops planning | later deploy/ops safety packet | plan controlled live mutation and in-flight shutdown proof | no unplanned production-destructive action |

...
0. APOLLO launch expansion fork, if chosen next
   ...
   - ATHENA Real Ingress Bridge is closed locally/runtime in ATHENA: the
     CLI-first report proves source truth, accepted-presence truth, identity
...
`ATHENA Real Ingress Bridge` is now closed in ATHENA repo/runtime truth:
`athena edge ingress-bridge` produces stable JSON/text eligibility proof over
source/accepted presence, identity presence, facility/zone/node scope,
timestamps, projection replay reasons, and source-pass session facts. Source
pass/fail truth remains immutable, accepted-presence truth remains separate,

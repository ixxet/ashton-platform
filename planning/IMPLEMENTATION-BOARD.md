...
`Phase 3B.10 bounded staff schedule controls` and `Phase 3B.11 competition
command foundation` are now
...
staff schedule controls, plus `Phase 3B.11` APOLLO-backed competition ops
...
0. APOLLO launch expansion fork, if chosen next
   - the canonical operating doc is
     `../apollo/docs/launch-expansion-audit.md`
   - 3B.11 has closed the command/readiness/CLI/Themis ops foundation only
   - do result lifecycle/trust, rating extraction/policy/audit, match tiers,
     consensus, disputes, and OpenSkill dual-run before public competition
     stakes
...
1. next Phase 3B planning fork after closed `Phase 3B.11`
   ...
   - APOLLO now also owns competition command/outcome/readiness contracts plus
     service-backed competition CLI parity over existing competition behavior
   - Themis now owns the internal `/ops/bookings` staff workspace and
     ...
     outcomes without fake competition state; it still denies `/api/v1/public/*`
     through its broad APOLLO proxy
   - choose 3B.12 result lifecycle/trust next if continuing launch expansion;
...
Themis repo/runtime truth: APOLLO owns shared competition command/outcome DTOs,
readiness/capability checks, dry-run plan output, and service-backed CLI parity
over existing competition behavior. Themis owns the internal `/ops/competition`
shell over those APOLLO contracts and does not own competition state. Hestia
remains unchanged. Deployed truth is unchanged.

The active next ladder is now a post-`Phase 3B.11` planning fork:

| Order | Line | Repo focus | Release line | Purpose | Hard stop |
| --- | --- | --- | --- | --- | --- |
| 1 | `Phase 3B.12` competition lifecycle/result trust | `apollo`, with Themis only as APOLLO consumer if needed | next if launch expansion continues | make result mutation trustworthy before ratings or public stakes | no OpenSkill, disputes/corrections/voiding, public pages, tournaments, CP/badges/rivalry/squads, browser trusted-surface token, or Hestia expansion by implication |
| 2 | rating/OpenSkill ladder | `apollo` | later | extract current rating policy, add audit/versioning, then dual-run OpenSkill | no hard swap from populated legacy ratings, public rating claims, or ARES v2 before trust gates |
| 3 | analytics/tournament/public/game identity ladder | likely `apollo`, `themis`, then `hestia` only after gates | later | add analytics, tournament runtime, public competition readiness, and game identity in order | no public competition surface before privacy, result trust, rating, telemetry, scale, and compatibility gates |

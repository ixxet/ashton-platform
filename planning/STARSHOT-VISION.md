...
| `APOLLO backend/runtime` | current Tracer 28 repo/runtime closeout line plus the later `Phase 3 shared substrate B`, `Phase 3B.1`, `Phase 3B.4`, `Phase 3B.5`, `Phase 3B.6`, `Phase 3B.7`, `Phase 3B.8`, `Phase 3B.9`, `Phase 3B.10`, `Phase 3B.11`, `Phase 3B.12`, `Phase 3B.13`, `Phase 3B.14`, `Phase 3B.15`, `Phase 3B.16`, and `Phase 3B.17` lines are on `main`; deployed truth unchanged | sport registry, facility-sport capability mapping, queue/assignment/lifecycle truth, team/roster/session/match execution containers, canonical result identity, recorded/finalized/disputed/corrected/voided facts, correction supersession, finalized/corrected-only rating consumption, versioned legacy rating truth, internal OpenSkill comparison facts, ARES v2 queue intent and match-preview proposal facts, internal competition analytics stat events/projections, staff/internal tournament bracket/seed/team-snapshot/match-binding/advancement/event facts, sport-and-mode-separated legacy active ratings, session-scoped standings, self-scoped member stats, the planner/deterministic-coaching plus bounded nutrition/helper and facility-scoped presence surfaces, the bounded competition role/authz substrate, the first real scheduling substrate, read-only ops overview, internal request-first booking truth, approved linked-reservation cancellation, public request intake API truth, public-safe availability hints, opaque public receipt/status/message lookup, pending request edit, approved replacement requests, and bounded staff-created schedule blocks are real while OpenSkill read-path switch, dashboard-first analytics, public profiles/stats/scouting, carry coefficient, public tournaments, public competition reads, CP/badges/rivalry/squads, instant booking, public self-edit/rebook, broader customer self-service, payments, quotes, in-place approved mutation, recurring schedule policy, broad operating-hours editing, and broader staff product remain deferred | done |
...
| --- | --- | --- |
| 1 | keep `Phase 3 shared substrate B`, `Phase 3A.1`, `Phase 3A.2`, `Phase 3A.4`, `Phase 3B.1`, `Phase 3B.2`, `Phase 3B.4`, `Phase 3B.5`, `Phase 3B.6`, `Phase 3B.7`, `Phase 3B.8`, `Phase 3B.9`, and `Phase 3B.10` closed in repo/runtime truth | make later ops and booking work build on already-real scheduling, member-shell, Hestia frontend, member-safe calendar, APOLLO ops-read/booking/public-intake/status/edit-replacement/public-availability/staff-schedule truth, approved lifecycle truth, and Themis ops/booking/schedule shell truth instead of reopening finished lines |
| 2 | choose the next Phase 3B fork: public self-edit/rebook, recurrence, in-place approved mutation, or payment/quote planning depending on the next proven operational gap | move the active blocker beyond bounded staff schedule controls without backsliding into self-confirmed customer booking, payments, quotes, owner policy writes, broad admin role work, Hestia staff controls, gateway work, or deploy work |

...
| Planner / coaching / nutrition / presence / authz backend | current repo/runtime truth is real through the facility-scoped presence line on `main`; role/authz remains staged | planner, templates/loadouts, richer profile inputs, finished-workout coaching feedback, deterministic coaching proposals, typed nutrition inputs, meal-template/log truth, conservative nutrition ranges, bounded helper reads, and facility-scoped presence/tap-link/streak truth are now real on `main` while role/authz widening remains intentionally staged | medium |
...
| `APOLLO backend/runtime` | current Tracer 27 repo/runtime closeout line on `main`; deployed truth unchanged | sport registry, facility-sport capability mapping, queue/assignment/lifecycle truth, team/roster/session/match execution containers, immutable result capture, sport-and-mode-separated ratings, session-scoped standings, self-scoped member stats, and the planner/deterministic-coaching plus bounded nutrition/helper and facility-scoped presence surfaces are real while public competition reads and role/authz remain deferred | done |
...
| `Platform docs` | synced to current repo truth and release lines | control-plane planning truth now matches tagged Tracer 24 APOLLO truth plus current Tracer 27 APOLLO repo/runtime closeout truth without widening deployed claims | keep synced as work lands |
...
| Presence / tap-link / streak substrate | no | yes |
| Role/authz and staff boundary substrate | no | yes |
...
| 5 | treat `Tracer 23` through `Tracer 28` as backend product expansion, still without real frontend widening | keep the platform modular |

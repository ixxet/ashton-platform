...
| `APOLLO backend/runtime` | current Tracer 28 repo/runtime closeout line plus the later `Phase 3 shared substrate B`, `Phase 3B.1`, `Phase 3B.4`, `Phase 3B.5`, `Phase 3B.6`, `Phase 3B.7`, `Phase 3B.8`, `Phase 3B.9`, `Phase 3B.10`, `Phase 3B.11`, `Phase 3B.12`, `Phase 3B.13`, `Phase 3B.14`, `Phase 3B.15`, `Phase 3B.16`, `Phase 3B.17`, `Phase 3B.18`, `Phase 3B.19`, `Phase 3B.20`, and `Phase 3B.20.1` lines are on `main`; deployed truth unchanged | sport registry, facility-sport capability mapping, queue/assignment/lifecycle truth, team/roster/session/match execution containers, canonical result identity, recorded/finalized/disputed/corrected/voided facts, correction supersession, finalized/corrected-only rating consumption, versioned legacy rating truth, internal OpenSkill comparison facts, ARES v2 queue intent and match-preview proposal facts, internal competition analytics stat events/projections, staff/internal tournament bracket/seed/team-snapshot/match-binding/advancement/event facts, manager/internal safety report/block/reliability/audit facts, public-safe competition readiness/leaderboard contracts, public/member-safe game identity projections, sport-and-mode-separated legacy active ratings, session-scoped standings, self-scoped member stats, the planner/deterministic-coaching plus bounded nutrition/helper and facility-scoped presence surfaces, the bounded competition role/authz substrate, the first real scheduling substrate, read-only ops overview, internal request-first booking truth, approved linked-reservation cancellation, public request intake API truth, public-safe availability hints, opaque public receipt/status/message lookup, pending request edit, approved replacement requests, and bounded staff-created schedule blocks are real while OpenSkill read-path switch, dashboard-first analytics, public profiles/stats/scouting, carry coefficient, public tournaments, public/member safety UI, messaging/chat, broad public social graph, instant booking, public self-edit/rebook, broader customer self-service, payments, quotes, in-place approved mutation, recurring schedule policy, broad operating-hours editing, and broader staff product remain deferred | done |
| `Hestia customer shell` | public intake with availability hints at `/intake`, customer-safe receipt lookup at `/intake/status`, and authenticated member app under `/app/**` are on `main`; deployed truth unchanged | Hestia is the customer-facing shell without staff controls, payments, quotes, public self-edit/rebook, self-booking, or deployment claims | done |
...
| `Platform docs` | synced to current repo truth and release lines | control-plane planning truth now matches the later `athena v0.7.0` deploy line plus current APOLLO `v0.19.1`, shared substrate B, ops-read, request-first booking, approved booking lifecycle, public intake/status, and booking edit/replacement closeout truth without widening deployed claims | keep synced as work lands |
...
`Phase 3B.1` is now also closed in APOLLO repo/runtime truth on `main`: one
read-only ops overview composes APOLLO schedule truth with ATHENA current
...
Deployed truth is unchanged, and public request intake, instant booking,
payments, quotes, customer shell work, owner policy, admin role, gateway,
HERMES, and deploy remained deferred in that line.

`Phase 3B.5 approved booking lifecycle` is now closed in APOLLO and Themis
repo/runtime truth on `main`: APOLLO owns approved cancellation through the
existing trusted-surface-gated, expected-versioned `/cancel` transition,
atomically cancels the linked internal reservation block, and retains
booking-to-schedule linkage for audit. Themis exposes approved cancellation only
to manager/owner sessions and keeps supervisors read-only. Deployed truth is
unchanged.

`Phase 3B.6 public request entrypoint` is now closed in APOLLO, Themis, and
Hestia repo/runtime truth on `main`: APOLLO owns the public intake API,
...
and Themis repo/runtime truth on `main`: APOLLO owns opaque public
receipt/status/message lookup, Hestia owns the customer-safe `/intake/status`
surface, and Themis lets manager/owner staff save APOLLO's separate public-safe
message while supervisors remain read-only. Deployed truth is unchanged.

`Phase 3B.8 booking edit and replacement` is now closed in APOLLO and Themis
repo/runtime truth on `main`: APOLLO owns staff-side pending request edit,
approved replacement request lineage, rebook idempotency, and schedule
availability truth; Themis owns the internal manager/owner workflow while
supervisors remain read-only. Hestia remains customer-facing and unchanged.
Deployed truth is unchanged.

`Phase 3B.9 public availability/request calendar` is now closed in APOLLO and
Hestia repo/runtime truth on `main`: APOLLO owns the unauthenticated read-only
public-safe availability endpoint for booking options; Hestia `/intake` shows
requestable windows plus unavailable time hints. Submit is still request-only,
and staff approval still creates the only confirmed reservation. Deployed truth
is unchanged.

`Phase 3B.10 bounded staff schedule controls` is now closed in APOLLO and
Themis repo/runtime truth on `main`: APOLLO remains the schedule authority and
guards booking-linked reservations from generic schedule cancellation; Themis
adds internal supervisor read-only schedule viewing and manager/owner
single-instance schedule block create/cancel controls. Hestia remains
customer-facing and deployed truth is unchanged.

| Line | Repo focus | Release line | Purpose | Hard stop |
| --- | --- | --- | --- | --- |
| public self-edit/rebook | likely `apollo` plus `hestia`, with Themis review context later | later only if a privacy-safe request amendment contract is proven | let customers amend requests without exposing internal schedule truth | no direct schedule mutation, confirmed self-booking, payment flow, private labels, conflict truth, contact data, or deploy claim |
| payment/quote planning | planning-first unless a runtime contract is explicitly earned | later | map deposits, quotes, invoices, and cancellation policy without implying checkout readiness | no payment processor integration, no checkout UI, no deploy claim |
| recurrence or broader schedule policy | likely `apollo` plus `themis` only after a separate proof | later | add repeat or policy controls only when operating rules are explicit | no broad admin schedule editor, supervisor proposal workflow, tournament/session controls, or deploy claim by implication |
...
| --- | --- | --- |
| 1 | keep `Phase 3 shared substrate B`, `Phase 3A.1`, `Phase 3A.2`, `Phase 3A.4`, `Phase 3B.1`, `Phase 3B.2`, `Phase 3B.4`, `Phase 3B.5`, `Phase 3B.6`, `Phase 3B.7`, `Phase 3B.8`, `Phase 3B.9`, `Phase 3B.10`, `Phase 3B.11`, `Phase 3B.12`, `Phase 3B.13`, `Phase 3B.14`, `Phase 3B.15`, `Phase 3B.16`, `Phase 3B.17`, and `Phase 3B.18` closed in repo/runtime truth | make later ops, booking, competition, and safety work build on already-real scheduling, member-shell, Hestia frontend, member-safe calendar, APOLLO ops-read/booking/public-intake/status/edit-replacement/public-availability/staff-schedule/competition/safety truth, approved lifecycle truth, and Themis ops/booking/schedule/competition shell truth instead of reopening finished lines |
| 2 | choose the next Phase 3B fork: likely 3B.19 public competition readiness if launch expansion continues, or public self-edit/rebook, recurrence, in-place approved mutation, or payment/quote planning if operations outrank competition | move the active blocker beyond internal safety/reliability without backsliding into public competition exposure, self-confirmed customer booking, payments, quotes, owner policy writes, broad admin role work, Hestia staff controls, gateway work, SemVer governance, or deploy work |

...
| `APOLLO backend/runtime` | current Tracer 28 repo/runtime closeout line plus the later `Phase 3 shared substrate B`, `Phase 3B.1`, `Phase 3B.4`, `Phase 3B.5`, `Phase 3B.6`, and `Phase 3B.7` lines are on `main`; deployed truth unchanged | sport registry, facility-sport capability mapping, queue/assignment/lifecycle truth, team/roster/session/match execution containers, immutable result capture, sport-and-mode-separated ratings, session-scoped standings, self-scoped member stats, the planner/deterministic-coaching plus bounded nutrition/helper and facility-scoped presence surfaces, the bounded competition role/authz substrate, the first real scheduling substrate, read-only ops overview, internal request-first booking truth, approved linked-reservation cancellation, public request intake API truth, and opaque public receipt/status/message lookup are real while public competition reads, instant booking, public availability calendars, broader customer self-service, payments, quotes, edit/rebook, and broader staff product remain deferred | done |
| `Hestia customer shell` | public intake at `/intake`, customer-safe receipt lookup at `/intake/status`, and authenticated member app under `/app/**` are on `main`; deployed truth unchanged | Hestia is the customer-facing shell without staff controls, payments, quotes, edit/rebook, or deployment claims | done |
...
| `Platform docs` | synced to current repo truth and release lines | control-plane planning truth now matches the later `athena v0.7.0` deploy line plus current APOLLO `v0.19.1`, shared substrate B, ops-read, request-first booking, and approved booking lifecycle closeout truth without widening deployed claims | keep synced as work lands |
...
| --- | --- | --- | --- | --- |
| pending/approved booking edit or rebook flow | likely `apollo` plus `themis` | after closed `Phase 3B.7` if request changes are the next honest gap | model request changes without bypassing schedule conflict truth | no payment/quote sprawl, owner policy writes, broad admin blob, Hestia staff controls, or member self-booking UI |
| public availability/request calendar | likely `apollo` plus `hestia`, with Themis review context later | future booking-product line after status truth | show facility hours, unavailable approved slices, public event labels where allowed, and requestable time selection without making customer submits self-confirming | no contact info, raw conflict truth, quote promise, automatic approval, or deploy claim |
| bounded staff scheduling controls | likely `apollo` plus `themis` only if write rails are required | after closed `Phase 3B.7` | add staff schedule-control rails only if operators need them before more booking work | no public calendar by implication, owner policy writes, broad admin blob, ATHENA widening, gateway widening, or deploy claim |
...
| --- | --- | --- |
| 1 | keep `Phase 3 shared substrate B`, `Phase 3A.1`, `Phase 3A.2`, `Phase 3A.4`, `Phase 3B.1`, `Phase 3B.2`, `Phase 3B.4`, `Phase 3B.5`, `Phase 3B.6`, and `Phase 3B.7` closed in repo/runtime truth | make later ops and booking work build on already-real scheduling, member-shell, Hestia frontend, member-safe calendar, APOLLO ops-read/booking/public-intake/status truth, approved lifecycle truth, and Themis ops/booking shell truth instead of reopening finished lines |
| 2 | choose the next Phase 3B fork: pending/approved edit or rebook flow, public availability/request calendar, bounded staff scheduling controls, or payment/quote planning depending on the next proven operational gap | move the active blocker beyond public request intake and status follow-up without backsliding into self-confirmed customer booking, payments, quotes, owner policy writes, broad admin role work, Hestia staff controls, gateway work, or deploy work |

...
| Planner / coaching / nutrition / presence / authz backend | current repo/runtime truth is real through the conservative nutrition slice on `main`; later substrate lines still planned | planner, templates/loadouts, richer profile inputs, finished-workout coaching feedback, deterministic coaching proposals, typed nutrition inputs, meal-template/log truth, and conservative nutrition ranges are now real on `main` while presence and role/authz widenings remain intentionally staged | medium |
...
| `APOLLO competition runtime` | current Tracer 25 repo/runtime closeout line on `main`; deployed truth unchanged | sport registry, facility-sport capability mapping, queue/assignment/lifecycle truth, team/roster/session/match execution containers, immutable result capture, sport-and-mode-separated ratings, session-scoped standings, self-scoped member stats, and the planner/deterministic-coaching plus bounded nutrition substrate are real while public competition reads remain deferred | done |
...
| `Platform docs` | synced to current repo truth and release lines | control-plane planning truth now matches tagged Tracer 24 APOLLO truth plus current Tracer 25 APOLLO repo/runtime closeout truth without widening deployed claims | keep synced as work lands |

...
| `APOLLO backend/runtime` | current Tracer 28 repo/runtime closeout line plus the later `Phase 3 shared substrate B`, `Phase 3B.1`, `Phase 3B.4`, `Phase 3B.5`, `Phase 3B.6`, `Phase 3B.7`, `Phase 3B.8`, and `Phase 3B.9` lines are on `main`; deployed truth unchanged | sport registry, facility-sport capability mapping, queue/assignment/lifecycle truth, team/roster/session/match execution containers, immutable result capture, sport-and-mode-separated ratings, session-scoped standings, self-scoped member stats, the planner/deterministic-coaching plus bounded nutrition/helper and facility-scoped presence surfaces, the bounded competition role/authz substrate, the first real scheduling substrate, read-only ops overview, internal request-first booking truth, approved linked-reservation cancellation, public request intake API truth, public-safe availability hints, opaque public receipt/status/message lookup, pending request edit, and approved replacement requests are real while public competition reads, instant booking, public self-edit/rebook, broader customer self-service, payments, quotes, in-place approved mutation, direct staff schedule controls, and broader staff product remain deferred | done |
...
| --- | --- | --- | --- | --- |
| bounded staff scheduling controls | likely `apollo` plus `themis` only if write rails are required | after closed `Phase 3B.9` | add staff schedule-control rails only if operators need them before more booking work | no public self-booking by implication, owner policy writes, broad admin blob, ATHENA widening, gateway widening, or deploy claim |
...
| --- | --- | --- |
| 1 | keep `Phase 3 shared substrate B`, `Phase 3A.1`, `Phase 3A.2`, `Phase 3A.4`, `Phase 3B.1`, `Phase 3B.2`, `Phase 3B.4`, `Phase 3B.5`, `Phase 3B.6`, `Phase 3B.7`, `Phase 3B.8`, and `Phase 3B.9` closed in repo/runtime truth | make later ops and booking work build on already-real scheduling, member-shell, Hestia frontend, member-safe calendar, APOLLO ops-read/booking/public-intake/status/edit-replacement/public-availability truth, approved lifecycle truth, and Themis ops/booking shell truth instead of reopening finished lines |
| 2 | choose the next Phase 3B fork: bounded staff scheduling controls, public self-edit/rebook, in-place approved mutation, or payment/quote planning depending on the next proven operational gap | move the active blocker beyond availability-informed requests without backsliding into self-confirmed customer booking, payments, quotes, owner policy writes, broad admin role work, Hestia staff controls, gateway work, or deploy work |

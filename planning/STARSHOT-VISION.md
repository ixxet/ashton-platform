...
| `APOLLO backend/runtime` | current Tracer 28 repo/runtime closeout line plus the later `Phase 3 shared substrate B`, `Phase 3B.1`, `Phase 3B.4`, `Phase 3B.5`, `Phase 3B.6`, `Phase 3B.7`, `Phase 3B.8`, `Phase 3B.9`, and `Phase 3B.10` lines are on `main`; deployed truth unchanged | sport registry, facility-sport capability mapping, queue/assignment/lifecycle truth, team/roster/session/match execution containers, immutable result capture, sport-and-mode-separated ratings, session-scoped standings, self-scoped member stats, the planner/deterministic-coaching plus bounded nutrition/helper and facility-scoped presence surfaces, the bounded competition role/authz substrate, the first real scheduling substrate, read-only ops overview, internal request-first booking truth, approved linked-reservation cancellation, public request intake API truth, public-safe availability hints, opaque public receipt/status/message lookup, pending request edit, approved replacement requests, and bounded staff-created schedule blocks are real while public competition reads, instant booking, public self-edit/rebook, broader customer self-service, payments, quotes, in-place approved mutation, recurring schedule policy, broad operating-hours editing, and broader staff product remain deferred | done |

...
| Planner / coaching / nutrition / presence / authz backend | current repo/runtime truth is real through the bounded explanation/helper slice on `main`; later substrate lines still planned | planner, templates/loadouts, richer profile inputs, finished-workout coaching feedback, deterministic coaching proposals, typed nutrition inputs, meal-template/log truth, conservative nutrition ranges, and bounded helper reads are now real on `main` while presence and role/authz widenings remain intentionally staged | medium |
...
| `APOLLO competition runtime` | current Tracer 26 repo/runtime closeout line on `main`; deployed truth unchanged | sport registry, facility-sport capability mapping, queue/assignment/lifecycle truth, team/roster/session/match execution containers, immutable result capture, sport-and-mode-separated ratings, session-scoped standings, self-scoped member stats, and the planner/deterministic-coaching plus bounded nutrition and helper/explanation surfaces are real while public competition reads, presence, and role/authz remain deferred | done |
...
| `Platform docs` | synced to current repo truth and release lines | control-plane planning truth now matches tagged Tracer 24 APOLLO truth plus current Tracer 26 APOLLO repo/runtime closeout truth without widening deployed claims | keep synced as work lands |

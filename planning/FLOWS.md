...
| ARES v2 proposal foundation | Closed in Phase 3B.15 | APOLLO owns explicit queue intent facts and internal match-preview proposal facts with server-computed match quality, predicted win probability, and explanation codes; ARES remains proposal-only and does not own result, lifecycle, rating, booking, or public competition truth; OpenSkill read-path switch, analytics, tournament runtime, public/member matchmaking, and game identity remain deferred |

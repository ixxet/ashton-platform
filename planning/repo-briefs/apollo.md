...
competition lifecycle/result trust, and `Phase 3B.13` rating foundation.
It does not own raw presence truth, broad staff product workflows, instant
booking, public self-edit/rebook, payments, quotes,
in-place approved mutation, broad schedule policy, OpenSkill, ARES v2,
...
into OpenSkill, ARES v2, analytics, tournament runtime,
...
| results/history plus presence/authz runtime | Phase 3B.13 is the current repo/runtime competition trust/rating foundation line on `main` | Tracer 22 competition-history truth, Tracer 23 planner/profile substrate, Tracer 24 deterministic coaching substrate, Tracer 25 bounded nutrition substrate, Tracer 26 helper reads, Tracer 27 facility-scoped presence/tap-link/streak truth, Tracer 28 explicit role/authz plus actor attribution, Phase 3B.11 command/readiness/CLI foundation, Phase 3B.12 lifecycle/result trust, and Phase 3B.13 rating foundation are real on `main` while OpenSkill, ARES v2, analytics, tournament runtime, public competition reads, broader staff product, and deployment truth remain deferred |
...
| competition command/outcome/readiness contracts, canonical result identity, lifecycle/result facts, correction supersession, finalized/corrected-only rating consumption, and versioned legacy rating projections | Themis-owned competition truth, browser trusted-surface tokens, OpenSkill, ARES v2, analytics, tournament runtime, public competition surfaces, CP, badges, rivalry, or squads |
...
- `Phase 3B.13` now adds APOLLO-owned legacy rating foundation: explicit
  ...
  OpenSkill remains deferred to 3B.14, ARES v2 to 3B.15, analytics to 3B.16,
...
| next fork | 3B.14 OpenSkill dual-run if continuing launch expansion; otherwise a separately scoped booking/schedule/commercial packet only if explicitly reopened |

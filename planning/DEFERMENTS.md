...
| ARES v2 | Deferred to Phase 3B.15 after rating/trust substrate | `apollo` | Result trust, legacy rating foundation, and OpenSkill dual-run comparison exist, but ARES v2 is not closed | Dedicated ARES v2 packet starts and consumes APOLLO-owned rating evidence | No ARES v2 before explicit packet; no public stakes or read-path switch by implication | Phase 3B.14 |
| Analytics | Deferred to Phase 3B.16 after rating/ARES gates | `apollo` | OpenSkill dual-run is closed, but ARES v2 and analytics gates are not closed | OpenSkill dual-run and ARES v2 prerequisites are complete | No analytics-as-truth before rating trust | Phase 3B.14 |
| Tournament runtime | Deferred to Phase 3B.17 | `apollo`, `themis`; Hestia later only after public gate | Lifecycle/result trust, legacy rating foundation, and OpenSkill comparison exist, but tournament gates are not closed | Dedicated tournament runtime packet starts after trust/rating prerequisites | No tournament/session logic through generic schedule blocks or fake UI state | Phase 3B.14 |
| Public competition surfaces | Deferred to Phase 3B.19 behind launch-expansion gates | `apollo`, `hestia`, `themis` | Internal result/rating truth and OpenSkill comparison exist, but privacy, scale, frontend contract, compatibility, and read-path gates are not closed | `apollo/docs/launch-expansion-audit.md` gates are satisfied for the target surface | No public ratings, leaderboards, records, rivalry, tournament pages, or social display by implication | Phase 3B.14 |
| OpenSkill / ARES v2 public stakes | Planned behind rating trust substrate; OpenSkill dual-run closed in Phase 3B.14 | `apollo` | Current ratings are versioned legacy APOLLO projections over finalized/corrected canonical results, OpenSkill deltas are internal comparison facts, and ARES is preview-only | Later read-path/ARES packets explicitly accept comparison evidence | No hard swap from populated legacy ratings; no permanent competing Elo/OpenSkill public scores | Phase 3B.14 |
| Retention game layer | Deferred to Phase 3B.20 behind trust and privacy gates | `apollo`, `hestia` | CP, badges, squads, quests, and rivalry amplify whatever truth is underneath | Reporting/moderation, scale, and rating trust gates are closed | No badges, CP, public rivalry, squads, bounty, or public records from untrusted results | Phase 3B.14 |

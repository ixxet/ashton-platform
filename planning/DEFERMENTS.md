...
| ARES v2 | Deferred to Phase 3B.15 after rating/trust substrate | `apollo` | Result trust and legacy rating foundation exist, but OpenSkill dual-run is not closed | OpenSkill dual-run is complete and safe rating reads are proven | No ARES v2 before safe rating reads | Phase 3B.13 |
| Analytics | Deferred to Phase 3B.16 after rating/ARES gates | `apollo` | Result trust and legacy rating foundation exist, but OpenSkill dual-run and ARES v2 gates are not closed | OpenSkill dual-run and ARES v2 prerequisites are complete | No analytics-as-truth before rating trust | Phase 3B.13 |
| Tournament runtime | Deferred to Phase 3B.17 | `apollo`, `themis`; Hestia later only after public gate | Lifecycle/result trust and legacy rating foundation exist, but OpenSkill/tournament gates are not closed | Dedicated tournament runtime packet starts after trust/rating prerequisites | No tournament/session logic through generic schedule blocks or fake UI state | Phase 3B.13 |
| Public competition surfaces | Deferred to Phase 3B.19 behind launch-expansion gates | `apollo`, `hestia`, `themis` | Internal result/rating truth exists and 3B.13 rating foundation is real, but OpenSkill, privacy, scale, frontend contract, and compatibility gates are not closed | `apollo/docs/launch-expansion-audit.md` gates are satisfied for the target surface | No public ratings, leaderboards, records, rivalry, tournament pages, or social display by implication | Phase 3B.13 |
| OpenSkill / ARES v2 public stakes | Planned behind rating trust substrate; OpenSkill target Phase 3B.14 | `apollo` | Current ratings are versioned legacy APOLLO projections over finalized/corrected canonical results and ARES is preview-only | OpenSkill dual-run is complete and compared against the 3B.13 legacy baseline | No hard swap from populated legacy ratings; no permanent competing Elo/OpenSkill public scores | Phase 3B.13 |
| Retention game layer | Deferred to Phase 3B.20 behind trust and privacy gates | `apollo`, `hestia` | CP, badges, squads, quests, and rivalry amplify whatever truth is underneath | Reporting/moderation, scale, and rating trust gates are closed | No badges, CP, public rivalry, squads, bounty, or public records from untrusted results | Phase 3B.13 |

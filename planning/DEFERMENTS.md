...
| Competition result trust | Deferred to Phase 3B.12 | `apollo`, with Themis only as consumer if needed | Phase 3B.11 closes command/readiness/CLI foundation only | Result lifecycle/finalization packet starts | No hidden UI-owned result mutation, disputes/corrections/voiding, OpenSkill, or public stakes by implication | Phase 3B.11 |
| Analytics / ARES v2 | Deferred to Phase 3B.16 after rating/trust substrate | `apollo` | Command foundation exists, but trusted finalized results and rating policy are not ready | Result trust, rating extraction, and policy/audit substrate are complete | No analytics-as-truth before result/rating trust; no ARES v2 before safe rating reads | Phase 3B.11 |
| Tournament runtime | Deferred to Phase 3B.17 | `apollo`, `themis`; Hestia later only after public gate | Command foundation exists, but lifecycle/result/rating gates are not closed | Dedicated tournament runtime packet starts after trust/rating prerequisites | No tournament/session logic through generic schedule blocks or fake UI state | Phase 3B.11 |
| Public competition surfaces | Deferred to Phase 3B.19 behind launch-expansion gates | `apollo`, `hestia`, `themis` | Internal result/rating truth exists and 3B.11 command foundation is real, but result trust, disputes, privacy, scale, frontend contract, and compatibility gates are not closed | `apollo/docs/launch-expansion-audit.md` gates are satisfied for the target surface | No public ratings, leaderboards, records, rivalry, tournament pages, or social display by implication | Phase 3B.11 |
| OpenSkill / ARES v2 public stakes | Planned behind rating trust substrate; OpenSkill target Phase 3B.14 | `apollo` | Current ratings are legacy APOLLO projections and ARES is preview-only | Rating extraction, policy versioning, rating audit, golden cases, and OpenSkill dual-run are complete | No hard swap from populated legacy ratings; no permanent competing Elo/OpenSkill public scores | Phase 3B.11 |
| Retention game layer | Deferred to Phase 3B.20 behind trust and privacy gates | `apollo`, `hestia` | CP, badges, squads, quests, and rivalry amplify whatever truth is underneath | Result correction/dispute, reporting/moderation, scale, and rating trust gates are closed | No badges, CP, public rivalry, squads, bounty, or public records from uncorrectable/untrusted results | Phase 3B.11 |
...
| Competition command/readiness foundation | Closed as APOLLO-owned command/readiness/CLI plus Themis internal shell | `apollo`, `themis` | Deferred until APOLLO competition authz/trusted-surface truth and Themis internal shell existed | Reopen only for hardening or 3B.12 result lifecycle/trust | No OpenSkill, result trust/finalization, analytics, tournament runtime, public competition surface, CP/badges/rivalry/squads, browser trusted-surface token, or Hestia expansion by implication | Phase 3B.11 |

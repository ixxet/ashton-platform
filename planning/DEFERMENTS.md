...
| Competition result trust | Closed in Phase 3B.12 | `apollo`, with Themis as consumer | Phase 3B.12 closes canonical result identity, lifecycle/result facts, correction supersession, and finalized/corrected-only rating consumption | Reopen only for hardening or a later approval/proposal workflow | No UI-owned result truth, rating extraction, OpenSkill, public stakes, or Hestia expansion by implication | Phase 3B.12 |
| Analytics / ARES v2 | Deferred to Phase 3B.16 after rating/trust substrate | `apollo` | Result trust exists, but rating extraction, policy/audit, and ARES v2 gates are not closed | Rating extraction and policy/audit substrate are complete | No analytics-as-truth before rating trust; no ARES v2 before safe rating reads | Phase 3B.12 |
| Tournament runtime | Deferred to Phase 3B.17 | `apollo`, `themis`; Hestia later only after public gate | Lifecycle/result trust exists, but rating and tournament gates are not closed | Dedicated tournament runtime packet starts after trust/rating prerequisites | No tournament/session logic through generic schedule blocks or fake UI state | Phase 3B.12 |
| Public competition surfaces | Deferred to Phase 3B.19 behind launch-expansion gates | `apollo`, `hestia`, `themis` | Internal result/rating truth exists and 3B.12 result trust is real, but rating extraction, privacy, scale, frontend contract, and compatibility gates are not closed | `apollo/docs/launch-expansion-audit.md` gates are satisfied for the target surface | No public ratings, leaderboards, records, rivalry, tournament pages, or social display by implication | Phase 3B.12 |
| OpenSkill / ARES v2 public stakes | Planned behind rating trust substrate; OpenSkill target Phase 3B.14 | `apollo` | Current ratings are legacy APOLLO projections over finalized/corrected canonical results and ARES is preview-only | Rating extraction, policy versioning, rating audit, golden cases, and OpenSkill dual-run are complete | No hard swap from populated legacy ratings; no permanent competing Elo/OpenSkill public scores | Phase 3B.12 |
| Retention game layer | Deferred to Phase 3B.20 behind trust and privacy gates | `apollo`, `hestia` | CP, badges, squads, quests, and rivalry amplify whatever truth is underneath | Reporting/moderation, scale, and rating trust gates are closed | No badges, CP, public rivalry, squads, bounty, or public records from untrusted results | Phase 3B.12 |

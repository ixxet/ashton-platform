...
| APOLLO launch expansion planner | [`../apollo/docs/launch-expansion-audit.md`](../apollo/docs/launch-expansion-audit.md), then [`planning/IMPLEMENTATION-BOARD.md`](planning/IMPLEMENTATION-BOARD.md) | The APOLLO audit governs the post-current competition/rating/tournament/social expansion sequence; the implementation board keeps cross-repo coordination truth |
...
APOLLO launch-expansion note: the active source of truth for post-current
competition/rating/tournament/social sequencing is
...
| APOLLO backend/runtime | current Tracer 28 repo/runtime line plus the current `v0.19.1` hardening follow-up, the later `Phase 3 shared substrate B` line, `Phase 3B.1` ops read foundation, `Phase 3B.4` booking request runtime, `Phase 3B.5` approved booking lifecycle, `Phase 3B.6` public request intake API, `Phase 3B.7` customer status/message lookup, `Phase 3B.8` booking edit/replacement, `Phase 3B.9` public availability/request calendar, `Phase 3B.10` bounded staff schedule controls, `Phase 3B.11` competition command foundation, `Phase 3B.12` competition lifecycle/result trust, `Phase 3B.13` rating foundation, `Phase 3B.14` OpenSkill dual-run comparison, `Phase 3B.15` ARES v2 proposal foundation, `Phase 3B.16` competition analytics foundation, `Phase 3B.17` internal tournament runtime, `Phase 3B.18` social safety/reliability foundation, `Phase 3B.19` public competition readiness, `Phase 3B.20` game identity layer, and `Phase 3B.20.1` cohesion hardening are on `main`; deployed truth unchanged | current repo/runtime truth | sport registry, facility-sport capability mapping, authenticated internal HTTP queue/assignment/lifecycle truth, canonical result identity, recorded/finalized/disputed/corrected/voided result facts, correction supersession, finalized/corrected-only rating consumption, versioned legacy rating math, golden rating cases, rating audit events, projection watermarks, internal OpenSkill comparison rows/events with delta budgets and flags, explicit queue intent facts, internal ARES match-preview proposal facts, APOLLO-computed match quality, predicted win probability, explanation codes, internal competition stat events/projections with explicit projection version, confidence, sample size, computed time, source match/result, deterministic watermarks, staff/internal tournament containers, single-elimination bracket/seed/team-snapshot/match-binding/advancement facts, explicit advance reasons, tournament event facts, manager/internal safety report/block/reliability/audit facts, sport-and-mode-separated legacy active ratings, public-safe readiness/leaderboard contracts, public/member-safe CP/badge/rivalry/squad game identity projection contracts, session-scoped standings, self-scoped member stats, planner / exercise-library / template / richer-profile / deterministic-coaching plus bounded nutrition / helper-read / facility-scoped presence substrate, explicit role/authz with trusted-surface-gated staff competition mutations and durable actor attribution, APOLLO-owned schedule resources/resource-graph/typed-block truth, one read-only supervisor/manager/owner ops overview over APOLLO schedule plus ATHENA occupancy/analytics truth, internal and public booking request state/approval truth, public-safe availability hints, idempotent public submit, opaque public receipt/status/message lookup, approved linked-reservation cancellation, pending request edit, approved replacement requests, bounded manager/owner staff-created schedule blocks, and shared competition command/outcome/readiness contracts plus service-backed CLI parity are real without widening into dashboard-first analytics, public profiles/stats/scouting, carry coefficient, OpenSkill read-path switch, public tournaments, public/member safety UI, messaging/chat, broad public social graph, instant booking, public self-booking, public self-edit/rebook, broader customer self-service/status portals, payments, quotes, in-place approved booking mutation, broad operating-hours editing, recurring schedule policy, AI/LLM negotiation, SemVer governance, or deployment claims |
| Hestia customer shell | `Phase 3A.2`, `Phase 3A.3`, `Phase 3A.4`, `Phase 3B.6` intake consolidation, `Phase 3B.7` status lookup, `Phase 3B.9` public availability/request calendar, `Phase 3B.19` public competition readiness, `Phase 3B.20` APOLLO-provided game identity, and `Phase 3B.20.1` docs cohesion hardening closed on `main` | current repo/runtime truth | Hestia is the customer-facing shell with public booking-request intake and public-safe availability hints at `/intake`, customer-safe status lookup at `/intake/status`, public-safe competition readiness plus APOLLO-provided game identity at `/competition`, and authenticated member app under `/app/**`; it does not own booking truth, staff controls, public self-edit/rebook, self-booking, payments, quotes, CP/badge/rivalry/squad formulas, messaging/chat, broad public social graph, public tournaments, OpenSkill comparison truth, or deployment |
| Themis ops shell | `Phase 3B.2`, `Phase 3B.4`, `Phase 3B.5`, `Phase 3B.6` proxy/source hardening, `Phase 3B.7` public-message triage, `Phase 3B.8` booking edit/replacement, `Phase 3B.10` schedule controls, `Phase 3B.11` competition ops foundation, `Phase 3B.12` lifecycle/result states, and `Phase 3B.18` manager safety/reliability visibility closed on `main` | current repo/runtime truth | The privileged ops shell consumes APOLLO auth/session/profile, ops overview, booking request APIs, schedule APIs, and competition command/readiness/session/result/safety/reliability APIs; it renders APOLLO-backed dry-run/accepted/denied/unavailable competition states plus recorded/finalized/disputed/corrected/voided result truth and manager/internal safety/reliability facts without fake competition or safety state, `/api/v1/public/*` remains blocked through Themis, trusted-surface tokens stay server-side, and deployed truth is unchanged |
...
| Platform docs | current Milestone 2.0 control-plane closeout line on `main`; current ATHENA deploy truth plus later APOLLO shared substrate B, `Phase 3B.1` ops-read truth, Themis `Phase 3B.2` ops-shell truth, APOLLO/Themis `Phase 3B.4` booking request truth, APOLLO/Themis `Phase 3B.5` approved booking lifecycle truth, `Phase 3B.6` Hestia intake consolidation truth, `Phase 3B.7` customer status/message truth, `Phase 3B.8` APOLLO/Themis booking edit/replacement truth, `Phase 3B.9` APOLLO/Hestia public availability/request calendar truth, `Phase 3B.10` APOLLO/Themis bounded staff schedule-control truth, `Phase 3B.11` APOLLO/Themis competition command foundation truth, `Phase 3B.12` APOLLO/Themis lifecycle/result trust truth, `Phase 3B.13` APOLLO legacy rating foundation truth, `Phase 3B.14` APOLLO OpenSkill dual-run comparison truth, `Phase 3B.15` APOLLO ARES v2 proposal truth, `Phase 3B.16` APOLLO competition analytics truth, `Phase 3B.17` APOLLO internal tournament runtime truth, `Phase 3B.18` APOLLO/Themis safety/reliability truth, `Phase 3B.19` APOLLO/Hestia public competition readiness truth, `Phase 3B.20` APOLLO/Hestia game identity truth, and `Phase 3B.20.1` cohesion hardening truth are all later than that closeout note | current control-plane truth | front-facing ladder, repo/runtime patch hardening lines, and deployed-truth boundaries should now be read with the later bounded `athena v0.7.0` deploy widening, APOLLO scheduling/ops-read/booking lifecycle/public-intake/status/edit-replacement/public-availability/staff-schedule/competition-command/result-trust/rating-foundation/OpenSkill-dual-run/ARES-proposal/competition-analytics/internal-tournament/safety-reliability/public-competition-readiness/game-identity/cohesion-hardening closeouts, Hestia customer-facing intake/status/availability/public-competition/game-identity closeout, and Themis ops-shell/booking-workspace/message/edit-replacement/schedule-control/competition-ops/safety-reliability closeouts in mind |
...
`Phase 3B.1` is now closed in APOLLO repo/runtime truth: the internal
supervisor/manager/owner ops overview route composes APOLLO schedule truth with
ATHENA current occupancy and bounded aggregate analytics behind `ops_read`.
Deployed truth is unchanged, and the route is useful only when APOLLO has
`APOLLO_ATHENA_BASE_URL` configured.

`Phase 3B.2` is now closed in Themis repo/runtime truth: Themis is the first
privileged read-only ops shell, consumes APOLLO auth/session/profile plus the
APOLLO ops overview route, and keeps deployed truth unchanged. APOLLO, Hestia,
ATHENA, HERMES, gateway, and deploy were not touched.

`Phase 3B.4` is now closed in APOLLO and Themis repo/runtime truth: APOLLO owns
internal booking request persistence, state transitions, availability preview,
and conflict-aware approval into linked internal reservation blocks; Themis owns
the internal `/ops/bookings` staff workspace. Deployed truth is unchanged, and
public request intake, instant booking, payments, quotes, customer shell work,
owner policy, admin role, gateway, HERMES, and deploy remained deferred in that
line.

`Phase 3B.5 approved booking lifecycle` is now closed in APOLLO and Themis
repo/runtime truth: APOLLO owns approved cancellation through the existing
trusted-surface-gated, expected-versioned `/cancel` transition, atomically
cancels the linked internal reservation block, and retains booking-to-schedule
linkage for audit. Themis exposes the action only to manager/owner sessions and
keeps supervisors read-only. Deployed truth is unchanged.
...
Hestia, and Themis repo/runtime truth: APOLLO owns opaque public
receipt/status/message lookup, Hestia owns the customer-safe `/intake/status`
surface, and Themis lets manager/owner staff save APOLLO's separate
public-safe message while supervisors remain read-only. Deployed truth is
unchanged.
...
read-only. Hestia remains customer-facing and separate. Deployed truth is
unchanged.
...
requestable windows plus unavailable time hints. Submit remains request-only,
staff approval still creates the only confirmed reservation, Themis remains
untouched, and deployed truth is unchanged.

`Phase 3B.10 bounded staff schedule controls` is now closed in APOLLO and
Themis repo/runtime truth: APOLLO remains the schedule authority, exposes the
existing schedule block contract with a manage hint, and refuses generic
schedule cancellation of booking-linked reservations; Themis adds an internal
schedule page where supervisors are read-only and manager/owner users can
create single-instance supported blocks or cancel eligible future
schedule-managed non-reservation blocks. Hestia remains customer-facing and
deployed truth is unchanged.

`Phase 3B.15 ARES v2` is now closed in APOLLO repo/runtime truth:
APOLLO records explicit queue intent facts and internal ARES match-preview
proposal facts over trusted APOLLO projections, computes match quality,
predicted win probability, and explanation codes server-side, and keeps ARES
out of match lifecycle/result/rating ownership. Themis and Hestia remain
untouched, deployed truth is unchanged, and OpenSkill read-path switch,
dashboard-first analytics, public profiles/stats/scouting, carry coefficient,
tournament runtime, public competition surfaces, CP/badges/rivalry/squads, and
game identity were still deferred at that closure.

`Phase 3B.16 Competition Analytics Foundation` is now closed in APOLLO
repo/runtime truth: APOLLO owns internal derived stat events and analytics
projections over finalized/corrected canonical results plus legacy active
rating facts. Themis, Hestia, and deployed truth are unchanged. Dashboard-first
analytics, public profiles/stats/scouting, carry coefficient, tournament
runtime, public competition surfaces, CP/badges/rivalry/squads, and game
...
| game sessions | bounded internal execution truth, results/history, public readiness/leaderboard reads, and public/member-safe game identity projections are real locally |
| leaderboards | public-safe readiness leaderboards are real through APOLLO projections |
| social graph / rivalries | broad social graph is not real; 3B.20 ships only redacted rivalry identity facts from APOLLO game identity projections |
...
| `apollo` | Member auth, profile state, visit ingest and close, derived eligibility, explicit lobby membership, deterministic read-only match preview, workout runtime, deterministic recommendation reads, planner substrate, bounded competition execution/history runtime, competition command/readiness foundation, scheduling, ops-read, booking request runtime, bounded staff schedule controls, public competition readiness, and game identity projections | `ashton-proto`, `athena` | Auth, profile state, visit lifecycle, derived eligibility, lobby membership, deterministic match preview, workout/recommendation/planner truth, bounded competition truth, command/outcome/readiness contracts, CLI competition command parity, schedule truth, ops-read truth, internal booking approval/cancellation/edit/replacement, bounded staff schedule controls, public request intake API truth, public-safe availability hints, public receipt/status/message lookup, public-safe competition readiness/leaderboards, and public/member-safe CP/badge/rivalry/squad game identity projections are real on `main` | [README](https://github.com/ixxet/apollo/blob/main/README.md) |
| `hestia` | Customer-facing frontend shell with public `/intake`, public `/intake/status`, public `/competition`, and authenticated `/app/**` | `apollo` | Public request intake with public-safe availability hints, public-safe receipt status lookup, public competition readiness plus APOLLO-provided game identity, and member app shell are real on `main`; Hestia does not own booking truth, staff controls, payment/quote flows, public self-edit/rebook, CP/badge/rivalry/squad formulas, or deploy claims | [README](https://github.com/ixxet/hestia/blob/main/README.md) |
| `themis` | Privileged internal ops shell | `apollo` | Supervisor/manager/owner ops overview, booking workspace, bounded schedule workspace, and APOLLO-backed competition ops foundation are real on `main`; manager/owner users can save APOLLO's separate public-safe message field, edit eligible pending/open requests, create approved replacement requests, cancel approved bookings, create supported one-off staff schedule blocks, cancel eligible future schedule-managed non-reservation blocks, and run APOLLO-supported competition command dry-runs/live commands where APOLLO permits; supervisors remain read-only or APOLLO-readiness-limited, and `/api/v1/public/*` remains blocked through Themis | [README](https://github.com/ixxet/themis/blob/main/README.md) |
...
| `athena` | `v0.5.1` shipped; the Tracer 18 facility-truth line plus the `v0.6.1` hardening follow-up are on `main`; `v0.7.0` is now shipped and live as the bounded storage/analytics deployment line, and the later `v0.7.x` repo/runtime closeout is already on `main`; deploy truth remains separate | no active ladder line; separate deploy packet only if needed | ATHENA repo/runtime closeout is already done, and any later deploy repin or broader ATHENA widening must stay separate from later Phase 3B product packets |
| `apollo` | current Tracer 28 repo/runtime line plus the current `v0.19.1` hardening follow-up, the later `Phase 3 shared substrate B` line, the later `Phase 3A.1` member shell foundation line, the later `Phase 3A.3` member truth completion line, the later `Phase 3A.4` member-safe schedule calendar line, the later `Phase 3B.1` ops read foundation, the later `Phase 3B.4` booking request runtime, the later `Phase 3B.5` approved booking lifecycle, the later `Phase 3B.6` public request intake API, `Phase 3B.7` customer status/message lookup, `Phase 3B.8` booking edit/replacement, `Phase 3B.9` public availability/request calendar, `Phase 3B.10` bounded staff schedule controls, `Phase 3B.11` competition command foundation, `Phase 3B.12` lifecycle/result trust, `Phase 3B.13` rating foundation, `Phase 3B.14` OpenSkill dual-run comparison, `Phase 3B.15` ARES v2 proposal foundation, `Phase 3B.16` competition analytics foundation, `Phase 3B.17` internal tournament runtime, `Phase 3B.18` social safety/reliability foundation, `Phase 3B.19` public competition readiness, `Phase 3B.20` game identity layer, and `Phase 3B.20.1` cohesion hardening are on `main`; Tracer 24 remains tagged on `v0.15.0`; `v0.15.1` remains the narrow historical hardening patch line; deployed truth unchanged | post-`Phase 3B.20.1` fork | APOLLO now owns staff and public booking request runtime truth, public idempotency, source/channel, public-safe availability hints, public receipt/status/message lookup, linked reservation approval, approved cancellation of linked reservations, pending request edit, approved replacement requests, bounded staff-created schedule blocks, competition command/outcome/readiness/CLI foundation, canonical result identity, correction supersession, finalized/corrected-only rating consumption, versioned legacy rating foundation, internal OpenSkill comparison rows/events with delta budgets and flags, ARES v2 queue intent/match-preview proposal facts, internal competition analytics stat events/projections, staff/internal tournament bracket/seed/team-snapshot/match-binding/advancement/event facts, manager/internal safety report/block/reliability/audit facts, public-safe readiness/leaderboard projection contracts over trusted competition truth, public/member-safe CP/badge/rivalry/squad game identity projection contracts over public-safe competition projection rows, and hardened 3B.20.1 rivalry context plus row-scoped labels; OpenSkill read-path switch, dashboard-first analytics, public profiles/stats/scouting, carry coefficient, public tournaments, public/member safety UI, messaging/chat, broad public social graph, instant booking, public self-edit/rebook, broader customer self-service, payments, quotes, in-place approved booking mutation, recurring schedule policy, broad operating-hours editing, gateway, HERMES, AI/LLM negotiation, SemVer governance, and deploy remain deferred |
| `hestia` | `Phase 3A.2` standalone member frontend bootstrap is on `main`; pushed repo truth now names `Hestia`, keeps APOLLO as auth/session authority, records future privileged split to `Themis`, carries the closed `Phase 3A.3` thin member-truth consumption line, the closed `Phase 3A.4` Home schedule-outlook consumption line, the closed `Phase 3B.6` public intake consolidation at `/intake`, the closed `Phase 3B.7` receipt status lookup at `/intake/status`, the closed `Phase 3B.9` public availability/request calendar at `/intake`, the closed `Phase 3B.19` public competition readiness surface at `/competition`, the closed `Phase 3B.20` APOLLO-provided game identity surface at `/competition` and member competition UI, and `Phase 3B.20.1` docs cohesion hardening | post-`Phase 3B.20.1` fork | Hestia is the customer-facing shell with public booking-request intake and public-safe availability hints at `/intake`, public-safe status lookup at `/intake/status`, public-safe competition readiness plus APOLLO-provided game identity at `/competition`, and authenticated member app under `/app/**`; it must not gain staff controls, payment/quote flows, CP/badge/rivalry/squad formulas, messaging/chat, broad public social graph, public tournaments, OpenSkill comparison truth, public self-edit/rebook, self-booking, or deploy claims by implication |
| `themis` | `Phase 3B.2` standalone privileged ops shell foundation, `Phase 3B.4` internal booking request workspace, `Phase 3B.5` approved booking cancellation workflow, `Phase 3B.6` public-source/proxy-boundary hardening, `Phase 3B.7` public-safe customer message triage, `Phase 3B.8` booking edit/replacement, `Phase 3B.10` bounded staff schedule controls, `Phase 3B.11` competition ops foundation, `Phase 3B.12` lifecycle/result states, and `Phase 3B.18` manager safety/reliability visibility are on `main`; pushed repo truth names Themis, keeps APOLLO as auth/session/API authority, and consumes APOLLO ops overview plus booking request, schedule, competition command/readiness/session, result, and safety/reliability contracts | post-`Phase 3B.18` fork | Themis is the privileged internal ops shell; it remains blocked from proxying `/api/v1/public/*`, can save only APOLLO's separate public-safe message field, edit pending/open requests, create approved replacement requests, create bounded one-off schedule blocks, cancel eligible future schedule-managed non-reservation blocks, and render APOLLO-backed competition readiness/dry-run/command/result/safety/reliability states without fake competition or safety state; deployed truth is unchanged, and Hestia, ATHENA, HERMES, gateway, and deploy were not widened |

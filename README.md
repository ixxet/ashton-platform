...
`Phase 3B.14 OpenSkill dual-run` is now closed in APOLLO repo/runtime truth:
APOLLO computes internal OpenSkill comparison values beside versioned legacy
rating outputs from finalized/corrected canonical results only, records
legacy/OpenSkill deltas and explicit delta flags, and keeps the active rating
read path on the legacy projection. Themis and Hestia remain untouched,
deployed truth is unchanged, and OpenSkill read-path switch, ARES v2,
analytics, tournament runtime, public competition surfaces, CP/badges/rivalry/
squads, and game identity remain deferred.

The active next ladder is now post-`Phase 3B.14`:

| Order | Line | Repo focus | Purpose | Hard stop |
| --- | --- | --- | --- | --- |
| 1 | 3B.15 ARES v2 | `apollo` | use trusted legacy rating truth plus internal OpenSkill comparison evidence for explainable matchmaking inputs | no public rating claims, OpenSkill read-path switch, or analytics dashboard |
| 2 | Analytics ladder | `apollo` | use trusted/versioned rating outputs and ARES prerequisites after comparison | no analytics-as-truth before rating trust |
| 3 | tournament/public/game identity ladder | likely `apollo`, `themis`, then `hestia` only after gates | add tournament runtime, public competition readiness, and game identity in order | no public competition surface before privacy, result trust, rating, telemetry, scale, and compatibility gates |

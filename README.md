...
| APOLLO competition runtime | current Tracer 25 repo/runtime closeout line on `main`; deployed truth unchanged | current repo/runtime truth | sport registry, facility-sport capability mapping, authenticated internal HTTP queue/assignment/lifecycle truth, immutable result capture, sport-and-mode-separated ratings, session-scoped standings, self-scoped member stats, and the planner / exercise-library / template / richer-profile / deterministic-coaching plus bounded nutrition substrate are real without widening into public/social competition reads |
...
| Platform docs | current Tracer 25 control-plane closeout line on `main`; deployed truth unchanged | current control-plane truth | front-facing ladder, tagged Tracer 24 APOLLO truth, current Tracer 25 APOLLO repo/runtime closeout truth, and deployed-truth boundaries now tell the same story without widening deployed claims |
...
| `ashton-platform` | current Tracer 25 control-plane closeout line on `main`; deployed truth unchanged | `v0.0.33` | the conservative nutrition closeout line is now current, so the next control-plane line should track the explanation/helper split without widening deployed claims |

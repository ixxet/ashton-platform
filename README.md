...
APOLLO exposes versioned legacy rating math, golden cases, rating audit events,
source result IDs, rating event IDs, and deterministic projection watermarks
over finalized/corrected canonical results only. Themis renders APOLLO-backed
match/result states and result commands at `/ops/competition` without owning
result or rating truth. Hestia remains untouched, deployed truth is unchanged,
and OpenSkill, ARES v2, analytics, tournament runtime, public competition
surfaces, and game identity remain deferred.

The active next ladder is now post-`Phase 3B.13`:

| Order | Line | Repo focus | Purpose | Hard stop |
| --- | --- | --- | --- | --- |
| 1 | 3B.14 OpenSkill dual-run | `apollo` | run OpenSkill beside the 3B.13 legacy rating baseline for comparison only | no OpenSkill hard swap, public rating claims, or ARES v2 before dual-run evidence |
| 2 | ARES / analytics ladder | `apollo` | use trusted/versioned rating outputs for ARES v2 and analytics after comparison | no analytics-as-truth before rating trust |

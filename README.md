...
The active next ladder is a post-`Phase 3B.9` planning fork:

| Order | Line | Repo focus | Purpose | Hard stop |
| --- | --- | --- | --- | --- |
| 1 | bounded staff scheduling controls | likely `apollo` plus `themis` only if write rails are required | add bounded staff schedule-control rails if operators need direct controls before more booking work | no public self-booking by implication, owner policy writes, broad admin role work, ATHENA widening, gateway widening, or deploy claims |
| 2 | public self-edit/rebook | likely `apollo` plus `hestia`, with Themis review context later | let customers amend requests only after a separate privacy and conflict-boundary proof | no direct schedule mutation, confirmed self-booking, payment flow, private labels, conflict truth, contact data, or deploy claim |
| 3 | payment/quote planning | planning-first unless a runtime contract is explicitly earned | map deposits, quotes, invoices, and cancellation policy without implying checkout readiness | no payment processor integration, no checkout UI, no deploy claim |
...
| `themis` | Privileged internal ops shell | `apollo` | Supervisor/manager/owner ops overview and booking workspace are real on `main`; manager/owner users can save APOLLO's separate public-safe message field, edit eligible pending/open requests, create approved replacement requests, and cancel approved bookings; supervisors remain read-only, and `/api/v1/public/*` remains blocked through Themis | [README](https://github.com/ixxet/themis/blob/main/README.md) |

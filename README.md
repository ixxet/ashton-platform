...
| `apollo` | Member-facing application, competition runtime, planner substrate, deterministic coaching substrate, bounded nutrition substrate, facility-scoped presence substrate, and helper read layer | Real and executable, but intentionally narrow | First member auth, profile-state, visit-history, visit-closing, derived eligibility, explicit lobby membership, explicit workout runtime, deterministic recommendation slice, thin member shell, tagged Tracer 24 planner/coaching truth on `main`, the narrow `v0.15.1` hardening patch line, and the current Tracer 27 presence repo/runtime closeout line on `main`; deployed truth unchanged |
...
| APOLLO backend/runtime | current Tracer 27 repo/runtime closeout line on `main`; deployed truth unchanged | current repo/runtime truth | sport registry, facility-sport capability mapping, authenticated internal HTTP queue/assignment/lifecycle truth, immutable result capture, sport-and-mode-separated ratings, session-scoped standings, self-scoped member stats, and the planner / exercise-library / template / richer-profile / deterministic-coaching plus bounded nutrition / helper-read / facility-scoped presence substrate are real without widening into public/social competition reads |
...
| Platform docs | current Tracer 27 control-plane closeout line on `main`; deployed truth unchanged | current control-plane truth | front-facing ladder, tagged Tracer 24 APOLLO truth, current Tracer 27 APOLLO repo/runtime closeout truth, and deployed-truth boundaries now tell the same story without widening deployed claims |
...
| `ashton-platform` | current Tracer 27 control-plane closeout line on `main`; deployed truth unchanged | `v0.0.35` | the presence/tap-link/streak closeout line is now current, so the next control-plane line should track role/authz truth without widening deployed claims |

...
The active next ladder is now post-`Phase 3B.16`:

| Order | Line | Repo focus | Purpose | Hard stop |
| --- | --- | --- | --- | --- |
| 1 | 3B.17 Internal Tournament Runtime | `apollo`; Themis only if internal read-only visibility is explicitly needed | add internal tournament containers/runtime over trusted result/rating/analytics facts | no public tournament pages, fake brackets, or public stakes |
| 2 | public/game identity ladder | likely `apollo`, `themis`, then `hestia` only after gates | add public competition readiness and game identity in order | no public competition surface before privacy, result trust, rating, telemetry, scale, and compatibility gates |

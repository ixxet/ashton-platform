...
| `apollo` | Member-facing application, competition runtime, planner substrate, deterministic coaching substrate, bounded nutrition substrate, and helper read layer | Real and executable, but intentionally narrow | First member auth, profile-state, visit-history, visit-closing, derived eligibility, explicit lobby membership, explicit workout runtime, deterministic recommendation slice, thin member shell, tagged Tracer 24 planner/coaching truth on `main`, the narrow `v0.15.1` hardening patch line, and the current Tracer 26 helper repo/runtime closeout line on `main`; deployed truth unchanged |
...
| APOLLO competition runtime | current Tracer 26 repo/runtime closeout line on `main`; deployed truth unchanged | current repo/runtime truth | sport registry, facility-sport capability mapping, authenticated internal HTTP queue/assignment/lifecycle truth, immutable result capture, sport-and-mode-separated ratings, session-scoped standings, self-scoped member stats, and the planner / exercise-library / template / richer-profile / deterministic-coaching plus bounded nutrition and helper-read substrate are real without widening into public/social competition reads |
...
| Platform docs | current Tracer 26 control-plane closeout line on `main`; deployed truth unchanged | current control-plane truth | front-facing ladder, tagged Tracer 24 APOLLO truth, current Tracer 26 APOLLO repo/runtime closeout truth, and deployed-truth boundaries now tell the same story without widening deployed claims |
...
| `ashton-platform` | current Tracer 26 control-plane closeout line on `main`; deployed truth unchanged | `v0.0.34` | the explanation/helper closeout line is now current, so the next control-plane line should track presence/tap-link/streak truth without widening deployed claims |

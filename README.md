...
| `apollo` | Member-facing application, competition runtime, planner substrate, deterministic coaching substrate, bounded nutrition substrate, facility-scoped presence substrate, scheduling substrate, helper read layer, ops read foundation, and booking request runtime | Real and executable, but intentionally narrow | First member auth, profile-state, visit-history, visit-closing, derived eligibility, explicit lobby membership, explicit workout runtime, deterministic recommendation slice, thin member shell, tagged Tracer 24 planner/coaching truth on `main`, the narrow `v0.15.1` hardening patch line, the current Tracer 28 authz/staff-boundary repo/runtime closeout line, the later `Phase 3 shared substrate B` scheduling line, the later `Phase 3B.1` ops read foundation, and the `Phase 3B.4` request-first booking runtime on `main`; deployed truth unchanged |
| `themis` | Privileged internal ops frontend | Real and executable, but intentionally narrow | First supervisor/manager/owner ops shell over APOLLO auth/session/profile plus the APOLLO ops overview route is closed in repo/runtime truth; the `Phase 3B.4` internal booking request workspace is also closed over APOLLO booking APIs with supervisor read-only and manager/owner management; deployed truth unchanged |
...
| APOLLO backend/runtime | current Tracer 28 repo/runtime line plus the current `v0.19.1` hardening follow-up, the later `Phase 3 shared substrate B` line, `Phase 3B.1` ops read foundation, and `Phase 3B.4` booking request runtime are on `main`; deployed truth unchanged | current repo/runtime truth | sport registry, facility-sport capability mapping, authenticated internal HTTP queue/assignment/lifecycle truth, immutable result capture, sport-and-mode-separated ratings, session-scoped standings, self-scoped member stats, planner / exercise-library / template / richer-profile / deterministic-coaching plus bounded nutrition / helper-read / facility-scoped presence substrate, explicit role/authz with trusted-surface-gated staff competition mutations and durable actor attribution, APOLLO-owned schedule resources/resource-graph/typed-block truth, one read-only supervisor/manager/owner ops overview over APOLLO schedule plus ATHENA occupancy/analytics truth, and internal booking request state/approval truth are real without widening into public/social competition reads, public booking, payments, quotes, staff schedule editing, or deployment claims |
| Themis ops shell | `Phase 3B.2` and `Phase 3B.4` closed on `main` | current repo/runtime truth | The first privileged ops shell consumes APOLLO auth/session/profile, the APOLLO ops overview route, and APOLLO booking request APIs; supervisors are read-only for booking requests, manager/owner can create and decide requests with APOLLO trusted-surface proof, and deployed truth is unchanged |
...
| Platform docs | current Milestone 2.0 control-plane closeout line on `main`; current ATHENA deploy truth plus later APOLLO shared substrate B, `Phase 3B.1` ops-read truth, Themis `Phase 3B.2` ops-shell truth, and APOLLO/Themis `Phase 3B.4` booking request truth are all later than that closeout note | current control-plane truth | front-facing ladder, repo/runtime patch hardening lines, and deployed-truth boundaries should now be read with the later bounded `athena v0.7.0` deploy widening, APOLLO scheduling/ops-read/booking closeout, and Themis ops-shell/booking-workspace closeout in mind |
...
The active next ladder is a post-`Phase 3B.4` planning fork:

| Order | Line | Repo focus | Purpose | Hard stop |
| --- | --- | --- | --- | --- |
| 1 | staff scheduling controls | likely `apollo` plus `themis` only if write rails are required | add bounded staff schedule-control rails if operators need direct controls before more booking work | no public booking, owner policy writes, broad admin role work, ATHENA widening, gateway widening, or deploy claims |
| 2 | approved-booking cancellation/editing | likely `apollo` plus `themis` | manage approved internal reservation lifecycle if that is the next honest gap | no payments, quote sprawl, public booking, owner policy writes, broad admin role work, or Hestia/member booking UI |
| 3 | public booking entrypoint | APOLLO plus public/member surface only when earned | expose request entry only after internal request-first truth stays stable | no checkout/deposit/quote sprawl, no conflict-truth bypass, no gateway/deploy claims by implication |
...
| `apollo` | current Tracer 28 repo/runtime line plus the current `v0.19.1` hardening follow-up, the later `Phase 3 shared substrate B` line, the later `Phase 3A.1` member shell foundation line, the later `Phase 3A.3` member truth completion line, the later `Phase 3A.4` member-safe schedule calendar line, the later `Phase 3B.1` ops read foundation, and the later `Phase 3B.4` booking request runtime are on `main`; Tracer 24 remains tagged on `v0.15.0`; `v0.15.1` remains the narrow historical hardening patch line; deployed truth unchanged | no active ladder line; reopen only if a later Phase 3B packet proves a booking lifecycle, schedule-control, or public-entry contract gap | APOLLO now owns internal booking request runtime truth and linked reservation approval; public booking, payments, quotes, gateway, HERMES, and deploy remain deferred |
...
| `themis` | `Phase 3B.2` standalone privileged ops shell foundation and `Phase 3B.4` internal booking request workspace are on `main`; pushed repo truth names Themis, keeps APOLLO as auth/session/API authority, and consumes APOLLO ops overview plus booking request APIs | post-`Phase 3B.4` planning fork between staff scheduling controls, approved-booking cancellation/editing, and public booking entrypoint | Themis is the privileged internal ops shell; deployed truth is unchanged, and Hestia, ATHENA, HERMES, gateway, and deploy were not touched |

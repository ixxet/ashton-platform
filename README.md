...
| --- | --- | --- | --- | --- |
| 1 | `Phase 3A.2` | later frontend repo | frontend repo bootstrap over already-real APIs and the closed APOLLO member-shell foundation; reopen APOLLO only if the audit proves one tiny helper/extraction is needed | no fake booking UI, no ops-shell drift, and no APOLLO shell reopen as the main line |
| 2 | later APOLLO authz/admin widening only if earned | `apollo` | add a distinct `admin` role only if real runtime/product needs justify it, then let admin do owner-like graph work intentionally | no accidental role widening hidden inside frontend bootstrap |
...
| `apollo` | current Tracer 28 repo/runtime line plus the current `v0.19.1` hardening follow-up, the later `Phase 3 shared substrate B` line, and the later `Phase 3A.1` member shell foundation line are on `main`; Tracer 24 remains tagged on `v0.15.0`; `v0.15.1` remains the narrow historical hardening patch line; deployed truth unchanged | no active ladder line; reopen only if `Phase 3A.2` audit earns one tiny helper/extraction | `Phase 3A.1` is already closed; frontend repo bootstrap is now the active blocker and APOLLO should stay narrow unless that audit proves it needs one tiny supporting helper |

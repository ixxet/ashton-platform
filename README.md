...
| `themis` | Privileged internal ops frontend | Real and executable, but intentionally narrow | First supervisor/manager/owner ops shell over APOLLO auth/session/profile plus the APOLLO ops overview route is closed in repo/runtime truth; the `Phase 3B.4` internal booking request workspace, `Phase 3B.5` approved booking cancellation workflow, `Phase 3B.6` public-source triage/proxy-denial hardening, and `Phase 3B.7` public-safe customer message update are also closed over APOLLO booking APIs with supervisor read-only and manager/owner management; deployed truth unchanged |
...
| APOLLO backend/runtime | current Tracer 28 repo/runtime line plus the current `v0.19.1` hardening follow-up, the later `Phase 3 shared substrate B` line, `Phase 3B.1` ops read foundation, `Phase 3B.4` booking request runtime, `Phase 3B.5` approved booking lifecycle, `Phase 3B.6` public request intake API, and `Phase 3B.7` customer status/message lookup are on `main`; deployed truth unchanged | current repo/runtime truth | sport registry, facility-sport capability mapping, authenticated internal HTTP queue/assignment/lifecycle truth, immutable result capture, sport-and-mode-separated ratings, session-scoped standings, self-scoped member stats, planner / exercise-library / template / richer-profile / deterministic-coaching plus bounded nutrition / helper-read / facility-scoped presence substrate, explicit role/authz with trusted-surface-gated staff competition mutations and durable actor attribution, APOLLO-owned schedule resources/resource-graph/typed-block truth, one read-only supervisor/manager/owner ops overview over APOLLO schedule plus ATHENA occupancy/analytics truth, internal and public booking request state/approval truth, idempotent public submit, opaque public receipt/status/message lookup, and approved linked-reservation cancellation are real without widening into instant booking, public availability calendars, broader customer self-service/status portals, payments, quotes, edit/rebook, staff schedule editing, AI/LLM negotiation, or deployment claims |
| Hestia customer shell | `Phase 3A.2`, `Phase 3A.3`, `Phase 3A.4`, `Phase 3B.6` intake consolidation, and `Phase 3B.7` status lookup closed on `main` | current repo/runtime truth | Hestia is the customer-facing shell with public booking-request intake at `/intake`, customer-safe status lookup at `/intake/status`, and authenticated member app under `/app/**`; it does not own booking truth, staff controls, payments, quotes, or deployment |
| Themis ops shell | `Phase 3B.2`, `Phase 3B.4`, `Phase 3B.5`, `Phase 3B.6` proxy/source hardening, and `Phase 3B.7` public-message triage closed on `main` | current repo/runtime truth | The first privileged ops shell consumes APOLLO auth/session/profile, the APOLLO ops overview route, and APOLLO booking request APIs; supervisors are read-only for booking requests and public messages, manager/owner can create, decide, save public-safe messages, and cancel approved bookings with APOLLO trusted-surface proof, `/api/v1/public/*` remains blocked through Themis, and deployed truth is unchanged |
...
| Platform docs | current Milestone 2.0 control-plane closeout line on `main`; current ATHENA deploy truth plus later APOLLO shared substrate B, `Phase 3B.1` ops-read truth, Themis `Phase 3B.2` ops-shell truth, APOLLO/Themis `Phase 3B.4` booking request truth, APOLLO/Themis `Phase 3B.5` approved booking lifecycle truth, `Phase 3B.6` Hestia intake consolidation truth, and `Phase 3B.7` customer status/message truth are all later than that closeout note | current control-plane truth | front-facing ladder, repo/runtime patch hardening lines, and deployed-truth boundaries should now be read with the later bounded `athena v0.7.0` deploy widening, APOLLO scheduling/ops-read/booking lifecycle/public-intake/status closeout, Hestia customer-facing intake/status closeout, and Themis ops-shell/booking-workspace/message closeout in mind |
...
The active next ladder is a post-`Phase 3B.7` planning fork:

| Order | Line | Repo focus | Purpose | Hard stop |
| --- | --- | --- | --- | --- |
| 1 | pending/approved booking edit or rebook flow | likely `apollo` plus `themis` | model request changes without skipping schedule conflict truth, likely as cancel-and-new-request unless a narrower in-place proof is earned | no payments, quote sprawl, owner policy writes, broad admin role work, Hestia staff controls, or member self-booking UI |
| 2 | public availability/request calendar | likely `apollo` plus `hestia`, with Themis review context later | let customers choose requestable slices from facility hours and unavailable approved blocks while still submitting requests | no self-confirmed booking, public conflict truth, contact info, quote promise, or deploy claim |
| 3 | bounded staff scheduling controls | likely `apollo` plus `themis` only if write rails are required | add bounded staff schedule-control rails if operators need direct controls before more booking work | no public availability calendar by implication, owner policy writes, broad admin role work, ATHENA widening, gateway widening, or deploy claims |
| 4 | payment/quote planning | planning-first unless a runtime contract is explicitly earned | map deposits, quotes, invoices, and cancellation policy without implying checkout readiness | no payment processor integration, no checkout UI, no deploy claim |
...
| `apollo` | Member auth, profile state, visit ingest and close, derived eligibility, explicit lobby membership, deterministic read-only match preview, workout runtime, deterministic recommendation reads, planner substrate, bounded competition execution/history runtime, scheduling, ops-read, and booking request runtime | `ashton-proto`, `athena` | Auth, profile state, visit lifecycle, derived eligibility, lobby membership, deterministic match preview, workout/recommendation/planner truth, bounded competition truth, schedule truth, ops-read truth, internal booking approval/cancellation, public request intake API truth, and public receipt/status/message lookup are real on `main` | [README](https://github.com/ixxet/apollo/blob/main/README.md) |
| `hestia` | Customer-facing frontend shell with public `/intake`, public `/intake/status`, and authenticated `/app/**` | `apollo` | Public request intake, public-safe receipt status lookup, and member app shell are real on `main`; Hestia does not own booking truth, staff controls, payment/quote flows, edit/rebook, or deploy claims | [README](https://github.com/ixxet/hestia/blob/main/README.md) |
| `themis` | Privileged internal ops shell | `apollo` | Supervisor/manager/owner ops overview and booking workspace are real on `main`; manager/owner users can save APOLLO's separate public-safe message field, supervisors remain read-only, and `/api/v1/public/*` remains blocked through Themis | [README](https://github.com/ixxet/themis/blob/main/README.md) |
...
| `apollo` | current Tracer 28 repo/runtime line plus the current `v0.19.1` hardening follow-up, the later `Phase 3 shared substrate B` line, the later `Phase 3A.1` member shell foundation line, the later `Phase 3A.3` member truth completion line, the later `Phase 3A.4` member-safe schedule calendar line, the later `Phase 3B.1` ops read foundation, the later `Phase 3B.4` booking request runtime, the later `Phase 3B.5` approved booking lifecycle, the later `Phase 3B.6` public request intake API, and `Phase 3B.7` customer status/message lookup are on `main`; Tracer 24 remains tagged on `v0.15.0`; `v0.15.1` remains the narrow historical hardening patch line; deployed truth unchanged | post-`Phase 3B.7` planning fork | APOLLO now owns staff and public booking request runtime truth, public idempotency, source/channel, public receipt/status/message lookup, linked reservation approval, and approved cancellation of linked reservations; instant booking, public availability calendars, broader customer self-service, payments, quotes, edit/rebook, gateway, HERMES, AI/LLM negotiation, and deploy remain deferred |
| `hestia` | `Phase 3A.2` standalone member frontend bootstrap is on `main`; pushed repo truth now names `Hestia`, keeps APOLLO as auth/session authority, records future privileged split to `Themis`, carries the closed `Phase 3A.3` thin member-truth consumption line, the closed `Phase 3A.4` Home schedule-outlook consumption line, the closed `Phase 3B.6` public intake consolidation at `/intake`, and the closed `Phase 3B.7` receipt status lookup at `/intake/status` | post-`Phase 3B.7` planning fork | Hestia is the customer-facing shell with public booking-request intake at `/intake`, public-safe status lookup at `/intake/status`, and authenticated member app under `/app/**`; it must not gain staff controls, payment/quote flows, edit/rebook, or deploy claims by implication |
| `themis` | `Phase 3B.2` standalone privileged ops shell foundation, `Phase 3B.4` internal booking request workspace, `Phase 3B.5` approved booking cancellation workflow, `Phase 3B.6` public-source/proxy-boundary hardening, and `Phase 3B.7` public-safe customer message triage are on `main`; pushed repo truth names Themis, keeps APOLLO as auth/session/API authority, and consumes APOLLO ops overview plus booking request APIs | post-`Phase 3B.7` planning fork | Themis is the privileged internal ops shell; it remains blocked from proxying `/api/v1/public/*`, can save only APOLLO's separate public-safe message field for manager/owner users, deployed truth is unchanged, and Hestia, ATHENA, HERMES, gateway, and deploy were not widened |
